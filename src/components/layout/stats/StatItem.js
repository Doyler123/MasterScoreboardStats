...
function StatItem(props) {
  const { classes, title, titleColor, body, date, holeNumber, handleClickComp, handleClickHole } = props;
  return (
    <Grid item xs={props.xs} sm={props.sm} md={props.md} lg={props.lg} xl={props.xl} >
      <Card>
          <CardActionArea onClick={(event) => !!date ? handleClickComp(date) : handleClickHole(holeNumber) } disabled={!(date || holeNumber)}>
